...
class Dispatcher
{
    ...
    private function init()
    {
        if (!$this->isControllerExist()) {
            throw new NotConfiguredServiceException('Controller path is not specified or default namespace is not specified. Abort dispatching');
        }
        ...
        foreach ($files as $file) {
            ...
            try {
                ...
                foreach($reflection->getMethods() as $method) {
                    foreach ($method->getAttributes() as $key => $attribute) {
    ...
    private function cacheRoutesCollection(): void
    {
        ...
        $diff = array_diff($this->routes, $data);
    ...
    {
        $this->isCachedRoutes() ?
            $this->getCollectionRoutesFromCache()
            : $this->collectRoutes();
    ...
    private function getRootNamespace(): string
    {
        ...
        return substr($currentNamespace, 0, -(strlen($currentNamespace) - (strrpos($currentNamespace, '\\') + 1)));
    }
